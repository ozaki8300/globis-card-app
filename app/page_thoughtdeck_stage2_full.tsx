  ...
  const accentClass =
    perspective.accent === "blue"
      ? {
          ...
          hover: "hover:border-blue-500 hover:text-blue-300",
          bg: "bg-blue-950/10",
        }
      : perspective.accent === "purple"
        ? {
            text: "text-purple-400",
            border: "border-purple-900",
            borderStrong: "border-purple-700",
            hover: "hover:border-purple-500 hover:text-purple-300",
            bg: "bg-purple-950/10",
          }
        : {
            ...
            hover: "hover:border-neutral-600 hover:text-neutral-400",
    ...
    return (
      <section
        ...
        className={`cursor-pointer rounded-xl border p-4 transition ${
          isSelected
            ? `${selectedCardClass} ${accentClass.borderStrong}`
            : `${accentClass.border} ${accentClass.bg} hover:bg-white/5`
        }`}
      >
        <h3 className={`mb-1 text-[12pt] font-bold ${accentClass.text}`}>{section.title}</h3>
    ...
    return (
      ...
      >
        <div className="mb-2 flex items-start justify-between gap-3">
          <h3 className={`w-full text-[12pt] font-bold ${accentClass.text}`}>{card.title}</h3>
          <button
            onClick={(event) => {
        ...
        <div className="hidden flex-wrap items-center justify-end gap-3 lg:flex">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-neutral-800 bg-neutral-900/60 p-1">
            <button
              ...
              className={`rounded-lg border px-3 py-2 text-[11pt] ${accentClass.borderStrong} ${accentClass.text} ${accentClass.hover}`}
            >
              🔄 視点を変える
            </button>
...
<section className="no-scrollbar flex-1 overflow-auto p-4 max-lg:overflow-visible max-lg:p-4">
          ...
          {(perspective.left || perspective.center || perspective.right) && (
            <div className="mb-2 grid grid-cols-3 gap-4 px-1 text-[10.5pt] leading-none text-neutral-500 max-xl:grid-cols-1">
              <p>{perspective.left}</p>
              <p>{perspective.center}</p>
              <p>{perspective.right}</p>
            </div>
          )}
