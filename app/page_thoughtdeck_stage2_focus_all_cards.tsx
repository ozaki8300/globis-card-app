...
export default function Home() {
  ...
  const selectedFocusItem = focusItems.find((item) => item.id === selectedCardId) || null;
  ...
  }, [selectedCardId, selectedCard, selectedFocusItem, allCards, focusMode]);
      ...
      <div className="mx-auto flex min-h-[55vh] max-w-5xl flex-col justify-center">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="text-[18pt] font-bold text-neutral-100">{item.title}</h2>
          </div>
        ...
        <section className="rounded-2xl border border-blue-500/60 bg-blue-950/10 p-7 shadow-[0_0_0_1px_rgba(59,130,246,0.16)]">
          <div className="space-y-2 break-words text-[14pt] leading-8 text-neutral-100">
            {item.lines.length > 0 ? (
              item.lines.map((line, index) => <p key={index}>{renderInline(line)}</p>)
            ) : (
