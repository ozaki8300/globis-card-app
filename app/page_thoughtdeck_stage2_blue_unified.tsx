    ...
    return (
      <section
        ...
        className={`cursor-pointer rounded-xl border p-4 transition ${
          isSelected
            ? `${selectedCardClass} ${uiAccentClass.borderStrong}`
            : `${uiAccentClass.border} ${uiAccentClass.bg} hover:bg-white/5`
        }`}
      >
        <h3 className={`mb-1 text-[12pt] font-bold ${uiAccentClass.text}`}>{section.title}</h3>
    ...
    return (
      ...
      >
        <div className="mb-2 flex items-start justify-between gap-3">
          <h3 className={`w-full text-[12pt] font-bold ${uiAccentClass.text}`}>{card.title}</h3>
          <button
            onClick={(event) => {
        ...
        <div className="hidden flex-wrap items-center justify-end gap-3 lg:flex">
          ...
          <div className="flex items-center gap-2 rounded-xl border border-neutral-800 bg-neutral-900/60 p-1">
            <button
              ...
              className={`rounded-lg border px-3 py-2 text-[11pt] ${perspectiveButtonClass}`}
            >
              🔄 視点を変える
            </button>
