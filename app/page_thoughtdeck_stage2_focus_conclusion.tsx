...
export default function Home() {
  ...
  const sendSelectedCardToConclusion = () => {
    if (!selectedCard) return;
    setRaw((prev) => appendCardToConclusion(prev, selectedCard));
    showShortcutHint(`結論へ追加：${selectedCard.title}`);
  };
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "Enter" && focusMode && selectedCard) {
        event.preventDefault();
        sendSelectedCardToConclusion();
      }
  ...
  }, [selectedCardId, selectedCard, allCards, focusMode]);
  ...
  const renderFocusedCard = (card: Card) => {
    const areaLabel = card.area === "left" ? "事実" : card.area === "center" ? "解釈" : "論点";
      ...
      <div className="mx-auto flex min-h-[55vh] max-w-5xl flex-col justify-center">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h2 className="text-[18pt] font-bold text-neutral-100">{card.title}</h2>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-[10.5pt] text-neutral-500">
            <span className="rounded-full border border-neutral-700 px-3 py-1">{areaLabel}</span>
            <span>Enter：結論へ送る</span>
            <span>F / Esc：戻る</span>
          </div>
        </div>
        ...
        <section className="rounded-2xl border border-blue-500/60 bg-blue-950/10 p-7 shadow-[0_0_0_1px_rgba(59,130,246,0.16)]">
          <div className="space-y-2 break-words text-[14pt] leading-8 text-neutral-100">
            {card.lines.length > 0 ? (
              card.lines.map((line, index) => <p key={index}>{renderInline(line)}</p>)
            ) : (
              <p className="text-neutral-500">本文はInput欄に入力してください。</p>
            )}
          </div>
        </section>
        ...
        <div className="mt-4 flex flex-wrap justify-end gap-2">
          <button
            onClick={sendSelectedCardToConclusion}
            className="rounded-lg border border-blue-500/50 bg-blue-950/20 px-4 py-2 text-[11pt] text-blue-100 transition hover:bg-blue-900/30"
          >
            結論へ送る
          </button>
          <button
...
<section className="no-scrollbar flex-1 overflow-auto p-4 max-lg:overflow-visible max-lg:p-4">
          ...
          {topSections.length > 0 && !focusMode && <div className="mb-5 space-y-4">{topSections.map(renderOneColumnSection)}</div>}

          {focusMode && selectedCard ? (
            renderFocusedCard(selectedCard)
          ) : (
